//
var __extends = (this && this.__extends) || (function () {
    var extendStatics = Object.setPrototypeOf ||
        ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
        function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var Animal = (function () {
    function Animal(theName) {
        this.animalName = theName;
    }
    Animal.prototype.walk = function (distance) {
        console.log("My name is " + this.animalName + " i am walked " + distance + " meters.");
    };
    return Animal;
}());
var myAnimal = new Animal("Dog");
myAnimal.walk(10);
var Reptiles = (function (_super) {
    __extends(Reptiles, _super);
    function Reptiles(theName) {
        return _super.call(this, theName) || this;
    }
    Reptiles.prototype.run = function (distance) {
        console.log("My name is " + this.animalName + " I ran " + distance + " meters.");
    };
    return Reptiles;
}(Animal));
var myreptile = new Reptiles("Snake");
myreptile.run(100);
